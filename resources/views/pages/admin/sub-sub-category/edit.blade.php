...
@section('page-title','Edit sub sub category')
...
@section('content-header')
    <div class="content-header d-flex justify-content-between" style="width:100%">
        <div>
            <h2 class="content-title card-title">Edit {{$subSubCategory->name_de}}</h2>
        </div>
    </div>
@stop
...
@section('main-content')
    ...
    <form action="{{route('sub-sub-categories.update',$subSubCategory->id)}}" method="post" enctype="multipart/form-data" >
        @csrf
        @method('put')
        <div class="row">
            <div class="col-lg-8">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-4">
                                    ...
                                    <input type="text" name="name_ar" value="{{$subSubCategory->name_ar}}" placeholder="Enter category name arabic ..."
                            ...
                            <div class="col-md-6">
                                <div class="mb-4">
                                    ...
                                    <input type="text" name="name_de" value="{{$subSubCategory->name_de}}" placeholder="Enter category name german ..."
                                    ...
                                    <div>
                                        <select name="category" id="category" class="w-100 category">
                                            ...
                                            @foreach($categories as $category)
                                                <option @if($subSubCategory->subCategory->category->id == $category->id) selected @endif value="{{$category->id}}">{{$category->name_ar . ' - '. $category->name_de}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    @error('category_id')
                                    ...
                                    <div>
                                        <select name="sub_category" id="sub_category" class="w-100">
                                            ...
                                            @foreach($subCategories as $subCategory)
                                                <option @if($subCategory->id == $subSubCategory->subCategory->id) selected @endif value="{{$subCategory->id}}">{{$subCategory->name_ar . ' - '. $subCategory->name_de}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    @error('category_id')
                            ...
                            <div class="col-md-3">
                                ...
                                <label class="form-check mb-3">
                                    <input class="form-check-input"@if($subSubCategory->status) checked @endif type="checkbox" name="status">
